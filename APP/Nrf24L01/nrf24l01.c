...
u8 NRF24L01_Write_Reg(u8 reg, u8 value)
{
    u8 status;
    NRF_CS(0);                                     //使能SPI传输
    SpiReadWriteSameData(SPI1, &status, reg, 1);   //发送寄存器号
    SpiReadWriteSameData(SPI1, &status, value, 1); //写入寄存器的值
    NRF_CS(1);                                     //禁止SPI传输
    return (status);                               //返回状态值
}
//读取SPI寄存器值
...
u8 NRF24L01_Read_Reg(u8 reg)
{
    u8 reg_val;
    NRF_CS(0);                                     //使能SPI传输
    SpiReadWriteSameData(SPI1, &reg_val, reg, 1);  //发送寄存器号
    SpiReadWriteSameData(SPI1, &reg_val, 0xff, 1); //读取寄存器内容
    NRF_CS(1);                                     //禁止SPI传输
...
u8 NRF24L01_Read_Buf(u8 reg, u8 *pBuf, u8 len)
{
    u8 status;
    NRF_CS(0);                                   //使能SPI传输
    SpiReadWriteSameData(SPI1, &status, reg, 1); //发送寄存器值(位置),并读取状态值
